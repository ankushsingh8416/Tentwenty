...
const Navbar = () => {
  ...
  return (
    <nav>
      <div className="shadow-lg z-40 flex items-center justify-between px-10   bg-white fixed inset-x-0 top-0 mx-auto  py-5 lg:top-6 lg:mx-10 lg:px-14 lg:py-7">
        <div className="hidden links lg:flex gap-x-[40px] ">
          {navLinks.map((item, index) => (
            <p className="text-[#36485c]  cursor-pointer " key={index}>
              {item.name}
            </p>
          ))}
        </div>
        ...
        >
          <div>
            <div className="flex justify-between w-full items-center ">
              ...
              <div
                onClick={handleNav}
                className="rounded-full shadow-lg shadow-black-400 p-3 cursor-pointer"
              >
                <AiOutlineClose />
              </div>
            </div>
            <div className="border-b border-gray-300 my-4">
              <p className="w-[85%] md:w-[90%] py-4 h2-heading">TenTwenty</p>
            </div>
          </div>
